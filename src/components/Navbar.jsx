    ...
    return (
        <header className='w-full bg-transparent fixed top-0 left-0 right-0 transition-all ease-in duration-300'>
            <nav className={`py-4 lg:px-24 px-4 ${isSticky ? "sticky top-0 left-0 right-0 bg-blue-300" : ""}`}>
                    ...
                    <ul className='md:flex space-x-12 hidden' >
                        {
                           ...
                           className='block text-2xl text-black uppercase cursor-pointer hover:text-blue-700 font-bold'>{link}</Link> )
                        }
                    </ul> 
                    <div>
                ...
                <div className={`space-y-4 px-4 mt-16 py-7 bg-blue-700 ${isMenuOpen ? "block fixed top-0 right-0 left-30" : "hidden"}`}>
                    {
                        navItems.map(({link, path}) => <Link key={path} to={path} 
